...
def user_login(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.get_user()
            login(request, user)
            next_url = request.GET.get('next', 'song_list')
...
def song_list(request):
    ...
    return render(request, "songs/list_enhanced.html", context)
...
def admin_users_dashboard(request):
    """Custom admin dashboard for user management with PDF export"""
    if not request.user.is_staff:
        messages.error(request, "Admin access only.")
        return redirect("song_list")
